...
- **Agent availability**: `claudeAvailable` requires the ACP package and a stored OAuth token. `mistralVibeAvailable` requires both `vibe-acp --version` succeeding and a Mistral API key in `VIBE_HOME/.env`. `openCodeAvailable` requires OpenCode auth. `codexAvailable` requires `codex-acp --version` succeeding and a Codex API key in `CODEX_HOME/auth.json`.
...
- **MCP client config**: External MCP servers (stdio or HTTP) for Cursor / Claude; persisted as `mcp-clients.json`. Each server can be **enabled/disabled** in Settings without deleting it; disabled servers stay in config but are not probed or loaded for agents. After the API is listening, a background probe restores reachable **enabled** servers into `.cursor/mcp.json` and Claude `mcpServers` (same as before). Unreachable servers are skipped with a warning in **Settings → MCP** so they cannot crash startup or agent spawn. **`POST /api/settings/mcp-clients/check`** runs a dry-run and updates autoload from the results.
