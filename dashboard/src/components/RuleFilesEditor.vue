...
// classes
import { workspaceRulesApi, globalRulesApi, roleTemplatesApi } from '@/classes/api';

// types
import type { WorkspaceRuleFileSummary, RoleTemplate } from '@/@types/index';
...
const bIsGlobal = computed(() => props.source === 'global');
const workspaceId = computed(() => props.workspaceId ?? '');
const pageTitle = computed(() => (bIsGlobal.value ? 'Global rules' : 'Cursor rules'));
const pageDescription = computed(() =>
  bIsGlobal.value
    ? 'These files are injected into every agent prompt, in every workspace. Workspace rules win when they conflict.'
    : 'View and edit workspace rules stored in .cursor/rules for this workspace.'
);
const skeletonLabel = computed(() =>
  bIsGlobal.value ? 'Loading global rules' : 'Loading workspace rules'
);
const blankRuleContent = computed(() =>
  bIsGlobal.value
    ? '# Global rules\n\nThese instructions apply to every workspace.\n'
...
type RuleFilesClient = {
  list: typeof globalRulesApi.list;
  read: typeof globalRulesApi.read;
  update: typeof globalRulesApi.update;
  remove: typeof globalRulesApi.remove;
  rename: typeof globalRulesApi.rename;
};

function rulesApi(): RuleFilesClient {
  if (bIsGlobal.value) {
    return globalRulesApi;
  }
  const id = workspaceId.value;
  return {
    list: () => workspaceRulesApi.list(id),
    read: (filename: string) => workspaceRulesApi.read(id, filename),
    update: (filename: string, content: string) => workspaceRulesApi.update(id, filename, content),
    remove: (filename: string) => workspaceRulesApi.remove(id, filename),
    rename: (filename: string, newFilename: string) =>
      workspaceRulesApi.rename(id, filename, newFilename)
  };
}

async function fetchFiles(): Promise<void> {
  if (!bIsGlobal.value && !workspaceId.value) {
    return;
  }
...
async function loadFile(filename: string): Promise<void> {
  ...
  try {
    const { data } = await rulesApi().read(filename);
...
async function saveCurrent(): Promise<void> {
  if ((!bIsGlobal.value && !workspaceId.value) || !selectedFilename.value || !hasUnsavedChanges.value) {
    return;
  }
  bSaving.value = true;
  saveError.value = null;
  try {
    await rulesApi().update(selectedFilename.value, content.value);
    originalContent.value = content.value;
...
async function deleteFile(): Promise<void> {
  if ((!bIsGlobal.value && !workspaceId.value) || !fileToDelete.value) {
    return;
  }
  bDeleting.value = true;
  deleteError.value = null;
  try {
    await rulesApi().remove(fileToDelete.value);
...
async function createRule(): Promise<void> {
  ...
  if (newRuleMode.value === 'template') {
    ...
    initialContent = blankRuleContent.value;
  }

  bCreatingRule.value = true;
  try {
    await rulesApi().update(filename, initialContent);
    closeNewRuleModal();
    await fetchFiles();
    selectedFilename.value = filename;
    await loadFile(filename);
...
async function submitRename(): Promise<void> {
  if ((!bIsGlobal.value && !workspaceId.value) || !fileToRename.value) {
    return;
  }
  ...
  renameError.value = null;
  try {
    await rulesApi().rename(fileToRename.value, newName);
...
<template>
  ...
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
    <div v-if="!hideHeader">
      ...
      <p v-if="bIsGlobal" class="text-sm text-text-muted mt-1">
        {{ pageDescription }}
      </p>
    ...
    <div class="flex items-center gap-2" :class="{ 'ml-auto': hideHeader }">
      <button
        type="button"
  ...
  <!-- Loading -->
  <ListPageSkeleton v-if="bListLoading" variant="rows" :count="5" :label="skeletonLabel" />
  ...
  >
    <p v-if="bIsGlobal">No global rule files yet.</p>
    <p v-else-if="bRulesDirMissing">
    ...
    <p class="mt-2 text-xs text-text-muted">
      <template v-if="bIsGlobal">
        Use the <span class="font-semibold">New rule</span> button above to create one. They apply
        to every workspace.
      </template>
      <template v-else>
