/**
 * Pure helpers for agent model selection (quick Cursor presets, dimension
 * resolution, fallback labels). Single source — used by AgentModelPicker.
 */
...
export const MORE_MODEL_OPTION_VALUE = '__more_models__';
...
export interface ModelPickerState {
  ...
  thinkingList: string[];
  selectedThinkingName: string;
  contextList: string[];
  selectedContextName: string;
  bFastAvailable: boolean;
...
export function cursorPresetValue(label: string): string {
...
function optionMatchesCursorPreset(option: AgentModelOption, preset: CursorModelPreset): boolean {
  return (preset.modelNames as readonly string[]).includes(option.model);
}

export function findCursorPresetForOption(option: AgentModelOption): CursorModelPreset | null {
  return BASIC_CURSOR_MODEL_PRESETS.find((preset) => optionMatchesCursorPreset(option, preset)) ?? null;
}

export function findCursorPresetByLabel(label: string): CursorModelPreset | null {
...
export function parseConfiguredModelId(id: string): { baseId: string; config: Record<string, string> } | null {
...
export function fallbackModelOption(id: string): AgentModelOption {
  if (!id || id === 'auto') {
...
export function buildModelPickerState(
  ...
  const thinkingList = sortThinkingValues(
    uniqueValues(
      options.filter((option) => option.model === selectedModelName).map((option) => option.thinking)
    )
  );
...
export function buildCursorPresetOptions(options: AgentModelOption[]): ModelSelectOption[] {
...
export function buildVisibleModelOptions(args: {
  bCursorAgent: boolean;
  bShowAllCursorModels: boolean;
  picker: ModelPickerState;
  selected: AgentModelOption;
  modelSelectValue: string;
  presetOptions: ModelSelectOption[];
}): ModelSelectOption[] {
  const { bCursorAgent, bShowAllCursorModels, picker, selected, modelSelectValue, presetOptions } =
    args;

  if (!bCursorAgent) {
    return picker.modelList.map((model) => ({ value: model, label: model }));
  }

  const options = [...presetOptions];
  if (
    modelSelectValue.startsWith(CURSOR_CURRENT_VALUE_PREFIX) &&
    !options.some((option) => option.value === modelSelectValue)
  ) {
    ...
    modelSelectValue.startsWith(CURSOR_MODEL_VALUE_PREFIX) &&
    !options.some((option) => option.value === modelSelectValue)
  ) {
    options.push({ value: modelSelectValue, label: picker.selectedModelName });
  }

  if (bShowAllCursorModels) {
    const presetModelNames = new Set<string>(
...
export function hasHiddenModelOptions(args: {
  bCursorAgent: boolean;
  bShowAllCursorModels: boolean;
  ...
  const { bCursorAgent, bShowAllCursorModels, modelListLength, presetOptionsLength, options } = args;
  return (
    bCursorAgent &&
    !bShowAllCursorModels &&
    (modelListLength > presetOptionsLength ||
      options.some((option) => !findCursorPresetForOption(option)))
  );
}
...
export function resolveModelOption(
  ...
  return (
    ...
    options.find((option) => option.model === model && option.thinking === thinking) ??
...
export function resolveDefaultCursorModelOption(
  options: AgentModelOption[],
  preset: CursorModelPreset
): AgentModelOption | null {
  const matched = options.filter((option) => optionMatchesCursorPreset(option, preset));
  ...
  return resolveBestModelOption(matched, preferredThinking);
}
...
export function resolveDefaultModelOption(
  ...
  const matched = options.filter((option) => option.model === model);
  if (!matched.length) return null;

  const modelKey = normalizeModelName(model);
  const sortedThinking = sortThinkingValues(uniqueValues(matched.map((option) => option.thinking)));
  ...
  const contextValues = sortContextValues(
    uniqueValues(
      matched.filter((option) => option.thinking === thinking).map((option) => option.context)
    )
  );
  ...
  const bHasFastVariants = matched.some(
