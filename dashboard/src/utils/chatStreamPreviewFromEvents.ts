/**
 * Derives a short plain-text preview from agent stream event lines (same JSONL shape as the dashboard).
 * Used for session list previews when the assistant ends on a tool call.
 *
 ...
 *
 * Keep in sync with: app/api/src/classes/chatStreamPreviewFromEvents.ts
 */
...
/** See SessionChat.vue mergeAssistantTextIntoDisplayItems — keep behavior in sync. */
