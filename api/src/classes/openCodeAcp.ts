 ...
 *
 * Spawns `open-code-acp` as a subprocess for each prompt turn and communicates via
...
// ── Active session handler map (openCodeSessionId → handler) ──────────────────

const activeHandlers = new Map<string, AcpEventHandler>();

// ── Spawn open-code-acp and establish ACP connection ────────────────────────

async function spawnOpenCodeConnection(cwd: string): Promise<{
  ...
  proc.stderr?.on('data', (chunk: Buffer) => {
    ...
    if (text) console.error('[openCodeAcp] stderr:', text);
  });
...
export interface RunOpenCodeAcpParams {
  /** OpenCode session ID from a previous run — null to start a new session. */
...
export interface RunOpenCodeAcpResult {
  /** OpenCode session ID — persist in the DB for conversation continuity. */
  acpSessionId: string;
  stopReason?: string;
  error?: string;
}
...
export async function runOpenCodeAcp(
  ...
  const { conn, proc } = await spawnOpenCodeConnection(cwd);
  const killProc = () => {
    try {
      proc.kill();
    } catch {
  ...
  try {
    ...
    if (!acpSessionId) {
      ...
      try {
        ...
        console.warn('[openCodeAcp] loadSession failed, starting fresh session:', err);
